...
my ($gtf_dir, $block_size_N, $proportion_N, $account, $partition, $output_gtf_path, $output_log_folder, $container_cmd) = ( '', '', '', '', '', '', '', '');
...
if ( !$test_options || $gtf_dir eq '' || $block_size_N eq '' || $proportion_N eq '' || $account eq '' || $partition eq '' || $output_gtf_path eq '' || $output_log_folder eq '' || $container_cmd eq '') {
    print "\n\tInvalid or missing argument:
\te.g. $0 -gtf_dir=\$(RNASEQ_CLUSTER_GTF) -block_size_N=31 -proportion_N=0.05 -output_gtf_path=\$(RNASEQ_CLUSTER_GTF) -output_log_folder=\$(RNASEQ_CLUSTER_GTF) -cluster_R_cmd=\$(CLUSTER_R_CMD) -container_cmd=\$(CONTAINER_CMD) 
...
\t-container_cmd=s      Command used to load the container on the cluster
\n";
    exit 1;
}
...
while (my $file = readdir(DIR) ){
    ...
    my $script_plus_args = "$container_cmd R CMD BATCH --vanilla --slave '--args gene_gtf_path=\"$gtf_dir/$file\" output_gtf_path=\"$output_gtf_path\" genome_fasta_path=\"$gtf_dir/$genome_file\" N_block_size=$block_size_N N_proportion=$proportion_N' ./0Before/prepare_GTF.R";
