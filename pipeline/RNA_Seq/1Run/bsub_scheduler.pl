...
my ($sample_info_file, $output_log_folder, $index_folder, $fastq_folder, $kallisto_out_folder, $ens_release, $ens_metazoa_release, $data_host, $data_login, $data_passwd, $enc_passwd_file, $vit_fastqc_cmd, $vit_kallisto_cmd, $vit_R_cmd) = ('', '', '', '', '', '', '', '', '', '', '', '', '', '');
...
if ( !$test_options || $sample_info_file eq '' || $output_log_folder eq '' || $index_folder eq '' || $fastq_folder eq '' || $kallisto_out_folder eq '' || $ens_release eq '' || $ens_metazoa_release eq '' || $data_host eq '' || $data_login eq '' || $data_passwd eq '' || $enc_passwd_file eq '' || $vit_fastqc_cmd eq '' || $vit_kallisto_cmd eq '' || $vit_R_cmd eq ''){
