...
# if Kallisto needs to be launched
    ...
    if ( $libraryType eq 'SINGLE' ){
        ...
        for my $run ( @run_ids ){
            ...
            if ( $exp_id eq $GTEX_exp_id ){
                $kallisto_command .= '<(cat '.$fastqSamplePath.'/'.$run.'.fastq.gz.enc | openssl enc -aes-128-cbc -d -md md5 -pass file:'.$enc_passwd_file.') ';
            }
        }
    }
    elsif ( $libraryType eq 'PAIRED' ){
        ...
        for my $run ( @run_ids ){
            ...
            if ( $exp_id eq $GTEX_exp_id ){
                $kallisto_command .= '<(cat '.$fastqSamplePath.'/'.$run.'_1.fastq.gz.enc | openssl enc -aes-128-cbc -d -md md5 -pass file:'.$enc_passwd_file.') <(cat '.$fastqSamplePath.'/'.$run.'_2.fastq.gz.enc | openssl enc -aes-128-cbc -d -md md5 -pass file:'.$enc_passwd_file.') ';
            }
        }
    }
