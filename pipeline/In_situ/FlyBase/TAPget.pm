...
WHERE e.uniquename IN ($in_string)");
    $sth->execute()  or die "WARNING: ERR1 Unable to execute query\n";
    ...
    while ( my $hash_ref = $sth->fetchrow_hashref ){
        ...
        # Changed from version 1.0, which was designed to put stages from TAP statements with single stage in 'start_stage'. These now go in the new amp_stages array
        $exid_TAP{$ex_id}{start_stage} = $hash_ref->{cvt}  if ( $hash_ref->{ec_type} eq 'stage' && $hash_ref->{ectp_value} && $hash_ref->{ectp_value} eq 'FROM' );
