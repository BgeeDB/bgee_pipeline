...
FTP='ftp://ftp.ensembl.org/pub/current_mysql'
DIRS=2
...
if [ ${#DB} -lt 1 ]; then
    echo -e "\n\tMissing db name as argument\n\te.g. $0 -d ensembl_compara_84 [-r -a -s]\n\tOptions:\n\t-r    Remove the previous database release\n\t-a    Install all ensembl core & funcgen db/tables, not only gene tree requirements\n\t-s    Species list to only include (species names with '_', separated by ',' e.g. bos_taurus,homo_sapiens)\n"
    exit 2
fi
...
if [ $CLEAN -gt 0 ]; then
    ...
    for old_db in `find /var/lib/mysql/ -maxdepth 1 -type d -name \*[a-z]_$CLEAN\* -exec basename {} \;`
    do
...
##### Download MySQL schemata and required dumps for species core/funcgen database
echo "\t__ Downloading ensembl_core/ensembl_funcgen   current release MySQL schemata and required dumps __"
echo "\t___ WARNING ! You must have enough disk space, more than 180 GB (or < 45GB if not all tables) ___"
for species in "${array[@]}"
do
    if [ $ALL_DB -eq 1 ]; then
        wget -q -r --cut-dirs=$DIRS -nH -A "*" -R "CHECKSUMS.*" "$FTP/${species}_core_${DB##*_}_*"
        wget -q -r --cut-dirs=$DIRS -nH -A "*" -R "CHECKSUMS.*" "$FTP/${species}_funcgen_${DB##*_}_*"
    else
        wget -q -r --cut-dirs=$DIRS -nH -A "*_core_*.sql.gz, assembly.*, coord_system.*, dna.*, exon*, meta*, seq_region.*, transcript.*, transcript_stable_id.*, translation.*, translation_stable_id.*" -R "prediction_transcript.*, CHECKSUMS.*" "$FTP/${species}_core_${DB##*_}_*"
...
if [ $ALL_DB -eq 1 ]; then
    echo "\t___ WARNING ! You must have enough disk space: about 1400 GB ___"
else
    echo "\t___ WARNING ! You must have enough disk space: about 140 GB ___"
fi
...
echo 'Download disk space requirement: ' $INTDU
exit 0
