...
if( file.exists(metadata_info) ){
  library_metadata <- read.table(metadata_info, h=T, sep="\t")
} else {
...
## Check libraries
checkId <- data.frame(library_metadata$library_id[!(library_metadata$library_id %in% libDownloadJura$library_id)])
...
for (library in  unique(finalLibsToDown$library_id)) {
  print(library)
  # list all R1 and R2 files to run fastp QC
  r1_files <- character(0)
  ...
  for(run in run_accessions) {
    ...
    if (is.unsorted(as.integer(name_to_seq_length[,2])) ||
        as.integer(name_to_seq_length[1,2]) > as.integer(name_to_seq_length[2,2])) {
      stop("the sequence size in SRA files does not match the assumption that",
        ...
        " has smaller sequences than _3.fastq (if it exists). But was ", name_to_seq_length,
        " We make the assumption that files are ok but not named properly for run ", run, 
	". We reordered the files and Sequence length are now :")
      name_to_seq_length <- name_to_seq_length[order(as.integer(name_to_seq_length[,2])),]
      if (length(fastq_files_names) == 3) {
        warning("\tSequence size in I1 : ", name_to_seq_length[1,2],
	     "\n\tSequence size in R1 : ", name_to_seq_length[2,2],
	     "\n\tSequence size in R2 : ", name_to_seq_length[3,2])
      } else if (length(fastq_files_names) == 2) {
        warning("\tSequence size in R1 : ", name_to_seq_length[1,2],
             "\n\tSequence size in R2 : ", name_to_seq_length[2,2])
      } else {
    ...
    for (row_number in seq(nrow(name_to_seq_length))) {
      ...
      if (with_I1_file) {
        if (row_number == 1) {
          ...
          r1_files <- c(r1_files, new_file_name)
        } else if (row_number == 3) {
          new_file_name <- gsub("_3", "_R2", old_file_name);
          r2_files <- c(r2_files, new_file_name)
	}
      } else {
        if (row_number == 1) {
