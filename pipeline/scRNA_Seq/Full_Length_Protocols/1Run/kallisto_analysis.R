...
## This script is used to run kallisto and run the rna_seq_analysis per library

## Usage:
## R CMD BATCH --no-save --no-restore '--args scrna_seq_sample_info="scrna_seq_sample_info.txt" cells_folder="cells_folder" infoFolder="infoFolder" kallistoRun="yes"' kallisto_analysis.R kallisto_analysis.Rout
...
## kallistoRun --> Want to run pseudo-alignment and analysis (kallisto = yes) if just analysis (kallisto=no)
...
command_arg <- c("scrna_seq_sample_info", "cells_folder", "infoFolder", "kallistoRun")
...
kallisto <- function(cells_folder, libraryId, species){
  
  ## collect info library
  libraryTypeInfo <- annotation$libraryType[annotation$libraryId == libraryId]
  libraryReadLengthInfo <- annotation$readLength[annotation$libraryId == libraryId]
  ...
  ## write pseudo-alignment same directory of library
  outputLib <- file.path(cells_folder,libraryId)
  fastqFile <- list.files(path=outputLib, pattern = ".*gz$", full.names=T, recursive = TRUE)
  
  if (libraryTypeInfo == "SINGLE" && libraryReadLengthInfo >= 50){
    cat("The library ", libraryId, " is SINGLE and read length > 50", "\n")
    ...
    cat("The library ", libraryId, " is PAIRED and read length > 50", "\n")
...
analysisData <- function(abundance, gene2transcript, gene2biotype){
  ...
  kallisto_gene_count <- abundance[abundance$type == "intergenic", c(1, 5, 6, 7, 8, 9)]
  names(kallisto_gene_count)[1] <- "gene_id"
  ## For genic regions sum read counts, TPMs and FPKMs
  temp <- abundance[abundance$type == "genic", c(2, 5, 6, 7)]
...
for(species in unique(annotation$organism)){
  for(libraryId in annotation$libraryId[annotation$organism == species]){
    ...
    ## run kaliisto
    if (kallistoRun == "yes"){
      cat("Start kallisto for the library .... ", "\n")
      kallisto(cells_folder = cells_folder, libraryId = libraryId, species = species)
    } else {
        cat("Jump kallisto pseudo-alignment... Just do analysis", "\n")
      }
