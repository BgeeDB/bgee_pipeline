...
## Usage:
## R CMD BATCH --no-save --no-restore '--args NEW_scRNASeqLibrary="NEW_scRNASeqLibrary.tsv" raw_cells_folder="raw_cells_folder" output_info_file="output_info_file"' prepare_scrna_seq_sample_info.R prepare_scrna_seq_sample_info.Rout
...
## output_info_file --> path to the scrna_seq_sample_info.txt file containing information for each library
...
command_arg <- c("NEW_scRNASeqLibrary", "raw_cells_folder", "output_info_file")
...
tmpInfoFile <- paste0(output_info_file, ".tmp")
...
write.table(finalTable,file = output_info_file, quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE)
