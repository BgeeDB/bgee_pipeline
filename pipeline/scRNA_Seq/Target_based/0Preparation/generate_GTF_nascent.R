...
# "scRNASeqInfoFile" - full path to the sample info file created by the pipeline
...
command_arg <- c("gtf_dir", "metadata_file", "scRNASeqInfoFile")
...
for (species_id in species_ids) {
  ...
  if (length(grep(pattern = "Sn-scRNA-seq", x = unique(sample_info$RNAseqTags[sample_info$speciesId == species_id]))) >= 1) {
    ...
    gtf_gz_file <- gtf_gz_file[!grepl(pattern = "wo_intergenic.gtf.gz$|nascent.gtf.gz$|transcriptome.gtf.gz$", x = gtf_gz_file)]
