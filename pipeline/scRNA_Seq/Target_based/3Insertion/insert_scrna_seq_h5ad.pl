...
# Julien Wollbrett, created May 2025
# THIS SCRIPT IS NOT PART OF THE Bgee PIPELINE.
# It has been created to be able to insert data necessary to generate h5ad files.
# It does not insert calls at celltype level. It actually does not require to have generated the calls.
# It has been created to generate the h5ad file for the Allen brain mouse dataset without having the reference intergenic
# sequences for that species.
# This script insert target base experiments, libraries, annotatedSamples, individualSamples, 
# and individualSamples results. It does not insert the annotatedSample results.
...
my ($bgee_connector) = ('');
my ($targetBaseLibrary, $kallistoResults, $sexInfo)  = ('', '', '');
my ($singleCellExperiment, $bgeeLibraryInfo) = ('', '');

my ($pipelineReportFile, $filteredBarcodeDir) = ('', '');
my $extraMapping = '';
my $numberCore = 1;
...
if ( !$test_options || $bgee_connector eq '' || $targetBaseLibrary eq '' || $singleCellExperiment eq '' ||
    $bgeeLibraryInfo eq '' || $filteredBarcodeDir eq '' || $pipelineReportFile eq '' || $kallistoResults eq '' ||
    $sexInfo eq '' || $numberCore eq '' || $extraMapping eq ''){
