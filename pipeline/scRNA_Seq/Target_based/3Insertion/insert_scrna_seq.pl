...
# Julien Wollbrett, updated January 2026
# - Now the script insert also the number of cells per experiment in the rnaSeqExperiment table.
# - Do not anymore insert individualSampleGeneResult corresponding to raw counts per cell.
#   it wass too heavy and not used in the Java API. This info will only be available through the
#   h5ad files generated by the pipeline.
# - removed the reson for exclusion "absent calls not reliable" as it is not used anymore.

## XXX: if an annotation problem or a barcode file is not available (e.g compressed file for huge atlases) the script will fail
#        without inserting any rnaSeqLibaryAnnotatedSampleGeneResult for that experiment or library. The current version of the script
#        can not manage such scenario and would require to remove all already inserted libraries/experiments. In that context it would be
#        really useful to allow to detect libraries/annotated samples already inserted AND that have gene results associated to not
#        try to install them again. I had to update the script for that reason for Bgee 16.0. The update checked all libraryId with at least one gene result
#        and skipped insertion for them. This updated version has not been pushed because it was not safe for all contextes BUT :
## TODO: we should add an option to allow to restart insertion after an error if we are sure it had no side effects e.g --skipInsertedAfterError /!\ to use with caution

#####################################################################

# Define arguments & their default value
my ($bgee_connector) = ('');
my ($targetBaseLibrary, $callsResults, $sexInfo)  = ('', '', '');
...
if ( !$test_options || $bgee_connector eq '' || $targetBaseLibrary eq '' || $singleCellExperiment eq '' ||
    $bgeeLibraryInfo eq '' || $filteredBarcodeDir eq '' || $pipelineCallsSummary eq '' || $pipelineReportFile eq '' ||
...
for my $expId ( sort keys %processedLibraries ){
    ...
    $updateNumberOfCells->execute($numberOfBarcodes, $expId) or die $updateNumberOfCells->errstr;
    $updateNumberOfCells->finish;
    $insLib->finish;
